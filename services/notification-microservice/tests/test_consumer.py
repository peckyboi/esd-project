...
def test_order_disputed_notifies_freelancer(db_session):
    body = _make_body("OrderDisputed", {"order_id": 3, "freelancer_id": 202, "client_id": 5})
...
def test_order_cancelled_notifies_freelancer(db_session):
    body = _make_body("OrderCancelled", {"order_id": 4, "freelancer_id": 303, "client_id": 5})
...
def test_duplicate_event_is_ignored(db_session):
    body = _make_body(
        "OrderCreated",
        {"order_id": 5, "freelancer_id": 101, "client_id": 5},
...
def test_unknown_event_type_is_ignored(db_session):
    body = _make_body("SomeRandomEvent", {"order_id": 6, "freelancer_id": 101})
...
def test_missing_user_id_does_not_create_notification(db_session):
    body = _make_body("OrderCreated", {"order_id": 7})
